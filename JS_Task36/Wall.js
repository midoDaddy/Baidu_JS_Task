...
* @Last Modified time: 2017-08-22 23:15:53
*/
...
Wall.prototype = {
    ...
    //获取前进方向前面一格的坐标
    getForwardPos: function() {
        var y = parseInt(this.CFG.relatedCar.obj.style.top, 10),
            x = parseInt(this.CFG.relatedCar.obj.style.left, 10),
            deg = this.CFG.relatedCar.getDeg();
        switch(deg % 360) {
            case 90:
            case -270: x += 50;
                break;
    ...
    //粉刷car所指方向相邻的墙
    brushWall: function(order) {
        var color = order.replace(/^BRU\s/i, ''),
